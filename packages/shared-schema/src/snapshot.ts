 ...
 *
 * `.strict()` rejects unknown fields so adapters cannot smuggle raw content
 * (chat transcripts, code, etc.) through legacy v1 uploads. The recursive
 * denylist in `contentDenylist.ts` catches the same risk for any nested
 * shape that escapes the strict check.
 */
...
export const ModelBreakdownSchema = z
  .object({
    modelId: z.string(),
    requests: z.number().int().nonnegative(),
