  ...
  for (const u of DEMO_USERS) {
    ...
    for (let d = 0; d < 30; d++) {
      ...
      await prisma.usageDaily.upsert({
        ...
        update: { totalRequests: requests, promptTokens, outputTokens, totalTokens: promptTokens + outputTokens, premiumRequests },
        create: { userId: user.id, deviceId: device.id, date, totalRequests: requests, promptTokens, outputTokens, totalTokens: promptTokens + outputTokens, premiumRequests },
      });
    }
    ...
    await prisma.userStat.upsert({
      ...
      update: { totalRequests: totalReqs, promptTokens: totalPrompt, outputTokens: totalOutput, totalTokens: totalPrompt + totalOutput, premiumRequests: totalPremium, workspaceCount: rand(2, 8), sessionCount: rand(50, 300), topModel, lastSyncedAt: now },
      create: { userId: user.id, totalRequests: totalReqs, promptTokens: totalPrompt, outputTokens: totalOutput, totalTokens: totalPrompt + totalOutput, premiumRequests: totalPremium, workspaceCount: rand(2, 8), sessionCount: rand(50, 300), topModel, lastSyncedAt: now },
    });
    ...
    for (const repo of userRepos) {
      ...
      await prisma.repoStat.upsert({
        ...
        update: { requests: repoReqs, promptTokens: repoPT, outputTokens: repoOT, totalTokens: repoPT + repoOT, premiumReqs: +(repoReqs * 0.7).toFixed(1), topModel: randomModel(), isPublic: true, lastSyncedAt: now },
        create: { userId: user.id, repoIdentity: `github:${repo}`, displayMode: 'github', githubRepo: repo, requests: repoReqs, promptTokens: repoPT, outputTokens: repoOT, totalTokens: repoPT + repoOT, premiumReqs: +(repoReqs * 0.7).toFixed(1), topModel: randomModel(), isPublic: true, lastSyncedAt: now },
      });
    }
